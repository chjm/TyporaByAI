...
<div class="toolbar">
  <div class="group">
    <button class="btn" title="打开文件夹" onclick={onOpenFolder}>
    ...
    <button class="btn" title="打开文件" onclick={onOpenFile}>
    ...
    <button class="btn" title="新建文件" onclick={onNewFile}>
      <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M13 3h-2v8H3v2h8v8h2v-8h8v-2h-8V3z"/></svg>
    </button>
    <button class="btn" title="保存" onclick={onSave} disabled={!app.currentFile}>
  ...
  <div class="center">
    <div class="segmented" title="模式切换">
      <button class="seg" class:active={!app.sourceMode} onclick={() => onSetMode(false)}>浏览</button>
      <button class="seg" class:active={app.sourceMode} onclick={() => onSetMode(true)}>源码</button>
    </div>
  </div>

  <div class="group right">
    <button class="btn" class:active={app.showFindReplace} title="查找替换" onclick={onToggleFindReplace}>
    ...
    <button class="btn" title="切换主题" onclick={onToggleTheme}>
      {#if app.theme === 'light'}
    ...
    <button class="btn" class:active={app.showSidebar} title="文件树" onclick={onToggleSidebar}>
    ...
    <button class="btn" class:active={app.showOutline} title="大纲" onclick={onToggleOutline}>
